    ...
    public class DungeonArmor
    {
        ...
        public class PlayerDungeonArmorProfile
        {
            ...
            public PlayerDungeonArmorProfile(Mobile wearer, BaseArmor armorPiece)
            {
                ...
                if (Player == null)
                {
                    DungeonArmorDetail = new DungeonArmorDetail(AspectEnum.None, 1);
                    return;
                }
                ...
                AspectEnum currentDungeon = AspectEnum.None;
                int lowestTier = 1000;
                ...
                for (int a = 0; a < m_Layers.Count; a++)
                {
                    ...
                    if (dungeonArmor.Aspect == AspectEnum.None || dungeonArmor.TierLevel == 0)
                        MatchingSet = false;
                    ...
                    else
                    {
                        ...
                        if (currentDungeon == AspectEnum.None)
                        {
                            currentDungeon = dungeonArmor.Aspect;
                            lowestTierPiece = dungeonArmor;
                        }

                        else
                        {
                            if (currentDungeon != dungeonArmor.Aspect)
                                MatchingSet = false;
                        }
                ...
                if (armorPiece != null)
                {
                    currentDungeon = armorPiece.Aspect;
        ...
        public class DungeonArmorDetail
        {
            ...
            public DungeonArmorDetail(AspectEnum dungeonType, int tierLevel)
            {
                Dungeon = dungeonType;
                TierLevel = tierLevel;
                ...
                switch (Dungeon)
                {
                    case AspectEnum.None:
                    break;

                    case AspectEnum.Deceit:                      
                        DungeonName = "Deceit";
                        Hue = 1908;
                        EffectHue = Hue - 1;
        ...
        public class DungeonArmorGump : Gump
        {
            ...
            public DungeonArmorGump(BaseArmor baseArmor, Mobile from): base(10, 10)
            {
                ...
                if (baseArmor.Aspect == AspectEnum.None || baseArmor.TierLevel == 0) return;
