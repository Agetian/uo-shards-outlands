...
namespace Server.Movement
{
    public class MovementImpl : IMovementImpl
    {
        ...
        private bool Check(Map map, Mobile m, List<Item> items, List<Mobile> mobiles, int x, int y, int startTop, int startZ, bool canSwim, bool cantWalk, out int newZ)
        {
            ...
            bool ignoreDoors = (m_AlwaysIgnoreDoors || !m.Alive || m.Body.BodyID == 0x3DB || m.IsDeadBondedFollower);
        ...
        {
            return (!t.Alive || !m.Alive || t.IsDeadBondedFollower || m.IsDeadBondedFollower) || (t.Hidden && t.AccessLevel > AccessLevel.Player);
        }
